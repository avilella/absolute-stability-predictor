"""
Run SaProtΔG predictions on example structures from the examples/ directory.
...
def main():
    ...
    for pdb_id, (filename, chain, desc, length) in EXAMPLES.items():
        ...
        for i, model in enumerate(base_models, 1):
            try:
                ...
                val = round(avg[0], 3)
                b_preds.append(val)
                print(f"  SaProtΔG-base model {i}: {val:.3f} kcal/mol")
            except Exception as e:
                print(f"  SaProtΔG-base model {i} FAILED: {e}")
        ...
        for i, model in enumerate(aug_models, 1):
            try:
                ...
                val = round(avg[0], 3)
                a_preds.append(val)
                print(f"  SaProtΔG-aug  model {i}: {val:.3f} kcal/mol")
            except Exception as e:
                print(f"  SaProtΔG-aug  model {i} FAILED: {e}")

        b_avg, b_std = ensemble_stats(b_preds)
        a_avg, a_std = ensemble_stats(a_preds)
        print(f"  → SaProtΔG     : {b_avg:.3f} ± {b_std:.3f} kcal/mol")
        print(f"  → SaProtΔG-Aug : {a_avg:.3f} ± {a_std:.3f} kcal/mol")

        results[pdb_id] = {
            "description": desc,
            "chain":       chain,
            "length":      length,
            "saprot":     {"individual": b_preds, "mean": b_avg, "std": b_std},
            "saprot_aug": {"individual": a_preds, "mean": a_avg, "std": a_std},
        }
    ...
    for pdb_id, (filename, chain, desc, length) in EXAMPLES.items():
        ...
        for key, models in [("saprot", base_models), ("saprot_aug", aug_models)]:
            scans = []
    ...
    out_path = os.path.join(_ROOT, "paper_example_results_saprotdg.json")
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n\nDone! Results saved to {out_path}")
    print(f"\n{'PDB':<22} {'SaP':>7} {'SaP-Aug':>9}")
    print("-" * 40)
