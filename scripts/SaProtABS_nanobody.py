...
pdb_ls_path = "/home/jupyter-yehlin/Pairformer/nanobody_boltz2/nanobody_boltz2_results"
...
for pdb_file in os.listdir(pdb_ls_path):
    if pdb_file.endswith(".cif"):
        ...
        PDB_NAME = pdb_file.split(".cif")[0]
        CHAIN_ID = 'A'

        # Run predictions for each model and collect results
        predictions = []
        for i, model in enumerate(models, 1):
            pred_dg_per_res, pred_dg_avg, combined_seq = SaProtdG_predict(model, pdb_path, CHAIN_ID)
            pred_value = pred_dg_avg[0]
            
            predictions.append(pred_value)
            print(f"🧬 Model {i}: {PDB_NAME} Predicted ΔG (kcal/mol) using {WEIGHT_FILES[i-1]}: {pred_value:.2f}")
...
# Save results to CSV
results_df.to_csv('/home/jupyter-yehlin/ESM3_SaProt_dG/model_predictions_nanobody.csv', index=False)
print("\nResults saved to model_predictions_nanobody.csv")